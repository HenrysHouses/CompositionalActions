...
namespace HH.CompositionalActions
{
    ...
    using HH.CompositionalActions.Networking;
    using HH.Actors.Contracts;
    // using HH.Dare.GameState;
    using UnityEngine;

    [Serializable]
    public abstract class NetworkedActionInvoker
    {
...
//
//         protected NetworkedContextFactory contextFactory;
//         protected IActionContext? initiatingContext;
//         protected IActionResult? currentSequenceResults;
//
//         public NetworkedActionInvoker(NetworkBehaviour executor)
//         {
//             this.executor = executor;
//             history = new Stack<IAction>(20);
//             initiatingContext = null;
//             currentSequenceResults = null;
//             contextFactory = ConstructContextFactory();
// #if UNITY_EDITOR
//             Executor = executor;
//             DebugActionHistory = ActionHistory.ToArray(); // # NOTE This is the real code, for testing im making a dummy array to override this in the editor
// #endif
//         }
//
// #if UNITY_EDITOR
//         public Action<ExecutionState, NetworkedActionInvoker> EditorExecutionStateHook { get; set; } = (state, invoker) => { };
...
//
//         [field: SerializeField] public NetworkBehaviour Executor { get; protected set; } // exposing variables for the debugger
//
...
//
//         public virtual bool Invoke(ActionSequence newSequence, ActionContextNetworked context)
//         {
//             // NetworkRunner.trygetpla
//             // context.ExecutingActor.PlayerID.
//             return InvokeSequence(newSequence, context);
//         }
//
//         public abstract NetworkedContextFactory ConstructContextFactory();
//
//         /// <summary>
...
//         /// <param name="networkContext">How the action sequence was started.</param>
//         /// <returns>is true when a new sequence begins invocation</returns>
//         protected bool InvokeSequence(ActionSequence newSequence, ActionContextNetworked networkContext)
//         {
//             Debug.LogError("this is not imeplemented. passes null into context");
//             Debug.Break();
//             IActionContext actionContext = contextFactory.CreateContextFrom(null, networkContext);
//
...
//             IEnumerator invocation = Execute(newSequence.CopyActions(), actionContext, state);
//
//             actionContext.ActorCallback.OnComplete += ClearCurrentSequenceContext;
...
// #endif
//             actionContext.ActorCallback.MarkExecuting();
//
//             // true if the sequence just started invoking
//             return initiatingContext == actionContext;
//         }
//
